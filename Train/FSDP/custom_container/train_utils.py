...
def train(model, train_dataloader,eval_dataloader, tokenizer, optimizer, lr_scheduler, gradient_accumulation_steps, train_config, fsdp_config=None, local_rank=None, rank=None):
    ...
    log_dir = "/root/log"
    tensorboard_id = os.getenv("AIP_TENSORBOARD_RESOURCE_NAME").split("/")[-1]
    tensorboard_experiment_name = "llama2-ft-" + os.getenv("CLOUD_ML_JOB_ID")
    experiment_display_name = "llama2-ft-" + os.getenv("CLOUD_ML_JOB_ID") 
    ...
    for epoch in range(train_config.num_epochs):
        ...
        if train_config.run_validation:
            ...
            if train_config.enable_fsdp:
                if rank==0:
                    writer.add_scalar('Perplexity/eval', eval_ppl, epoch)
                    writer.add_scalar('Loss/eval', eval_epoch_loss, epoch)
                    writer.flush()
            else:
                writer.add_scalar('Perplexity/eval', eval_ppl, epoch)
                writer.add_scalar('Loss/eval', eval_epoch_loss, epoch)
